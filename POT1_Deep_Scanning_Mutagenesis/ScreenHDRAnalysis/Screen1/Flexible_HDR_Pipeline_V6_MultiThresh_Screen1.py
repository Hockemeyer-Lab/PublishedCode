...
with open(r"/data/annika/POT1_Screen_VUS_Combined/Screen/Input/POT1_Prot.txt" , "r") as file:
    gene_seq = file.read().upper()
with open(r"/data/annika/POT1_Screen_VUS_Combined/Screen/Input/POT1_Prot.txt", "r") as file:
    prot_seq = file.read().upper()
with open(r"/data/annika/POT1_Screen_VUS_Combined/Screen/Input/POT1_dna_seq_just_exons.txt", "r") as file:
    protdnaseq = file.read().upper()

guide_ref = pd.read_excel('/data/annika/POT1_Screen_VUS_Combined/Screen/Input/POT1_sgRNA_Seq.xlsx')
exon_seqs_df = pd.read_excel('/data/annika/POT1_Screen_VUS_Combined/Screen/Input/POT1_Amplicon_Sequences.xlsx')

#Input path information
oldpath = r"/home/annika/RB_POT1_Data/CRISPRessoOutputs"
#Output path information (make directory if needed)
dpath_parental = r"/data/annika/POT1_Screen_VUS_Combined/Screen/Output"
if os.path.exists(dpath_parental) == False:
    os.mkdir(dpath_parental)

if HDR:    
    HDR_lib_design = pd.read_excel(r"/data/annika/POT1_Screen_VUS_Combined/Screen/Input/intermediate_file_testing_12bp_v16_with_tagging.xlsx")
    sgRNA_list = HDR_lib_design[['sgRNA # oPool Name',"sgRNA orientation ? 0 is for 5' to 3', 1 is the other strand",  'Exon #', 'sgRNA seq', "region around sgRNA that is exon"]].drop_duplicates(keep='first').reset_index(drop=True)
    #Add amplicon information to sgRNA list
    sgRNA_list['Amplicon'] = [guide_ref[guide_ref['sgRNA sequence'] == guide]['Amplicon'].tolist()[0] for guide in sgRNA_list['sgRNA seq']]
...
legend_df = pd.read_excel(r"/data/annika/POT1_Screen_VUS_Combined/Screen/Input/CombinedTotalLegendFile.xlsx")
